...
function imparte(list: Excursie[]) {
...
export default function Excursii({ initial }: { initial?: Excursie[] | null }) {
  const [upcomingExcursii, setUpcomingExcursii] = useState<Excursie[]>(() => (initial ? imparte(initial).upcoming : []));
  const [pastExcursii, setPastExcursii] = useState<Excursie[]>(() => (initial ? imparte(initial).past : []));
  ...
  useEffect(() => {
    ...
    const loadExcursii = async () => {
      try {
        ...
        const { upcoming, past } = imparte(list);
  ...
  return (
    <DarkPageShell
      pagina="Excursii"
      url="https://www.inpasidedans.ro/excursii"
      orbe={[
        '-top-40 left-1/3 h-[36rem] w-[36rem] bg-orange-500/15',
        'top-1/4 -right-32 h-[32rem] w-[32rem] bg-red-600/20',
        'bottom-0 -left-20 h-[28rem] w-[28rem] bg-orange-500/10',
      ]}
      fundal={<TopographicBackground />}
    >
      {/* HERO */}
      <section className="mt-10 md:mt-16 mb-14 md:mb-20">
        <div className="grid md:grid-cols-[1fr_auto] gap-10 items-center max-w-6xl mx-auto">
          <div>
            <DarkEticheta icon={Compass}>Călătorii cu pasiune</DarkEticheta>
      ...
      {upcomingExcursii.length > 0 && (
        <section className="mb-16 md:mb-20">
          <DarkAntetSectiune titlu="Următoarele excursii" subtitlu="Locuri rezervate · destinații confirmate" />
      ...
      {pastExcursii.length > 0 && (
        <section id="galerie-trecute" className="mb-16 md:mb-20 scroll-mt-20">
          <DarkAntetSectiune titlu="Excursii anterioare" subtitlu="Amintiri și destinații explorate împreună" icon={ImageIcon} />
