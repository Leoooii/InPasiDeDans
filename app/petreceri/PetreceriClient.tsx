...
function imparte(list: Petrecere[]) {
...
export default function Petreceri({ initial }: { initial?: Petrecere[] | null }) {
  const [upcomingPetreceri, setUpcomingPetreceri] = useState<Petrecere[]>(() => (initial ? imparte(initial).upcoming : []));
  const [pastPetreceri, setPastPetreceri] = useState<Petrecere[]>(() => (initial ? imparte(initial).past : []));
