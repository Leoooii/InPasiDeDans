  ...
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4 mt-4">
      {tarife.map((tarif) => (
        <div
          key={tarif.id}
          className="bg-white border-2 border-slate-100 rounded-2xl p-7 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
        >
          <h3 className="font-bold text-slate-900 mb-1">{tarif.titlu}</h3>
          {tarif.descriere && (
